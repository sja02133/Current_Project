...
bool CSEND_CONTROL::SendData(WCHAR type, WCHAR* data, int len, CTCP_SOCKET* pTCP_SOCKET)
{
	...
	pTCP_SOCKET->SetInitialize_CLIENT_INFO(pTCP_SOCKET->clientInfo);
	...
	if (MakeSendData(type, data, len, pTCP_SOCKET, pTCP_SOCKET->clientInfo->cSock)) {
		return true;
	}
	else
		return false;
